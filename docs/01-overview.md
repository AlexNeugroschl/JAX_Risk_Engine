...
2. **Price every trade in every scenario.** For each simulated future, calculate what
   ...
   payments) — see [Instruments: Interest Rate Swaps](04-instruments.md).
...
3. **Measure the risk.** Look at the full spread of "what-if" trade values across every
   ...
   bad is that worst 5%?"). See [Risk Statistics: VaR & ES](05-risk-statistics.md).
...
- **A research question about precision.** Computers can do math with different levels
  ...
  switching precision on and off (see [Adjustable Precision](02-architecture.md#adjustable-precision)).
- **Correctness against a known-good reference.** Rather than inventing new math, this
  ...
  is described more in [Architecture](02-architecture.md).
...
| Interest rate swap pricing | Prices one type of trade (interest rate swaps) across every simulated scenario | ✅ Working |
| Value at Risk / Expected Shortfall | Turns trade values into risk numbers | ✅ Working |
| Other trade types (options, etc.) | Not yet built | 🔜 Planned |
| Live API | Not yet built | 🔜 Planned |

## Where to go next

- **New to finance/math and want the plain-language version of how the math works?**
  Each deep-dive doc ([Market Simulation](03-market-simulation.md),
  [Instruments](04-instruments.md), [Risk Statistics](05-risk-statistics.md)) starts
  with a "Plain-language summary" section before getting into formulas.
- **Want to actually run the code?** See the [User Guide](06-user-guide.md).
...
- **Don't know what a term means?** See the [Glossary](glossary.md).
