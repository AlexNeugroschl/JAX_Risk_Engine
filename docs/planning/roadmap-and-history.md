...
| 11. TraderX API integration | FastAPI microservice wrapping the pricing pipeline (`engine/portfolio/request.py::price_portfolio` + `engine/api/`) — gRPC was evaluated and explicitly not chosen | ✅ Done — see [TraderX Integration Plan](traderX_integration/traderx-integration.md), [The Portfolio Entry Point](../reference/portfolio-entrypoint.md), [HTTP API](../reference/http-api.md) |
